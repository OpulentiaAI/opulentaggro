...
import type { ERPNextClientLike } from "./ic-billing-tools.js";
...
export type IcExtendedToolName = (typeof icExtendedToolDefinitions)[number]["name"];
...
export async function handleIcExtendedToolCall(
  client: ERPNextClientLike,
  toolName: IcExtendedToolName,
    ...
    switch (toolName) {
      ...
      case "ic_get_reconciliation_summary": {
        const result = await callMethod(
          client,
          TREASURY_PREFIX,
          "get_central_reconciliation_summary",
          { company: args?.company }
        );
        return textResult(result);
      }
      ...
      case "ic_triangular_sale": {
        if (!args?.selling_company || !args?.billing_company || !args?.customer || !args?.items) {
          throw new McpError(
            ErrorCode.InvalidParams,
            "selling_company, billing_company, customer, and items are required"
          );
        }
      ...
      case "ic_create_accrual": {
        if (
          ...
          throw new McpError(
            ErrorCode.InvalidParams,
            "company, counterparty_company, amount, debit_account, and credit_account are required"
          );
        }
...
export function isIcExtendedToolName(name: string): name is IcExtendedToolName {
