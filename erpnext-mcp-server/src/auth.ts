...
export function isNoAuthModeEnabled(): boolean {
  ...
  if (!baseUrl) {
    throw new Error(
      "ERPNEXT_NO_AUTH/MCP_NO_AUTH requires ERPNEXT_URL to be set to a localhost URL"
    );
  }
  if (!isLocalhostUrl(baseUrl)) {
    throw new Error(
      "ERPNEXT_NO_AUTH/MCP_NO_AUTH is only allowed when ERPNEXT_URL points to localhost (dev only)"
    );
  }
  return true;
}

export function devCredentials(): { user: string; password: string } {
  return {
...
export async function loginDevSession(axiosInstance: AxiosInstance): Promise<void> {
  ...
  throw new Error(
    `Dev session login failed for ${user}: ${JSON.stringify(response.data?.message ?? response.data)}`
  );
}
