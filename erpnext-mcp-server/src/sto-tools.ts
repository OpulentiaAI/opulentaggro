...
export const stoToolDefinitions = [
  {
    name: "sto_create",
    description:
      "Create an intercompany Stock Transfer Order (internal Purchase Order). Maps to SAP MM STO creation by Requestor.",
    inputSchema: {
      type: "object",
      properties: {
...
export type StoToolName = (typeof stoToolDefinitions)[number]["name"];
...
export async function handleStoToolCall(
  client: ERPNextClientLike,
  toolName: StoToolName,
...
export function isStoToolName(name: string): name is StoToolName {
