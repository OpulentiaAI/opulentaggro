...
### Stock Transfer Order (STO) Tools — Intercompany

Dedicated tools for intercompany stock transfer workflows (maps AgroFresh SAP STO process):

- `sto_create` — Create internal Purchase Order as STO
- `sto_submit` — Submit STO after approval
- `sto_approve_and_route` — Create intercompany Sales Order
- `sto_post_goods_in_transit` — Post delivery / goods in transit (643)
- `sto_create_ic_invoice` — Create IC Sales + Purchase invoices
- `sto_post_goods_receipt` — Post intercompany Purchase Receipt
- `sto_get_trace` — Trace full document chain
- `sto_three_way_match` — PO / PR / PI three-way match
- `sto_list` — List stock transfer orders

Requires ERPNext with `erpnext.intercompany.stock_transfer_order` API (see workspace `docs/erpnext-sto-mcp-setup.md`).

### Intercompany Billing Tools — Standalone AR/AP

Tools for invoicing across **multiple company pairs** without an STO context:

- `ic_list_accounts` — List configured intercompany pairs (internal Customer/Supplier links)
- `ic_create_sales_invoice` — Create Sales Invoice on seller (AR)
- `ic_create_purchase_invoice` — Create Purchase Invoice on buyer (AP)
- `ic_create_invoice_pair` — Create linked SI + PI atomically
- `ic_submit_invoice` — Submit SI and/or PI
- `ic_get_invoice_status` — Trace AR/AP posting status

Requires `erpnext.intercompany.intercompany_billing` API.
...
When `ERPNEXT_URL` is `http://localhost:8000` (or 127.0.0.1), set:

```bash
ERPNEXT_NO_AUTH=1
ERPNEXT_DEV_USER=Administrator
ERPNEXT_DEV_PASSWORD=<from config/demo-credentials.env>
```

The MCP server logs in via Frappe `/api/method/login` and uses session cookies.
Do **not** set `ERPNEXT_NO_AUTH` against remote or production hosts.
