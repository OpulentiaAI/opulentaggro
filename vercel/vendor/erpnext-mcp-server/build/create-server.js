...
export function createErpnextMcpServer(client) {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        if (isStoToolName(request.params.name)) {
            return handleStoToolCall(erpnext, request.params.name, request.params.arguments);
        }
        ...
        switch (request.params.name) {
            ...
            case "call_method": {
                ...
                const httpMethod = request.params.arguments?.http_method || "POST";
                if (!method) {
            ...
            case "submit_document": {
                ...
                try {
                    ...
                    if (!result || typeof result !== "object" || result.name == null || result.docstatus == null) {
                        throw new McpError(ErrorCode.InternalError, `Unexpected response from ERPNext while submitting ${doctype} ${name}`);
                    }
                    return {
                        content: [
                            {
            ...
            case "cancel_document": {
                ...
                try {
                    ...
                    if (!result || typeof result !== "object" || result.name == null || result.docstatus == null) {
                        throw new McpError(ErrorCode.InternalError, `Unexpected response from ERPNext while cancelling ${doctype} ${name}`);
                    }
                    return {
                        content: [
                            {
