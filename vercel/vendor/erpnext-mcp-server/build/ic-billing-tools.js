...
export const icBillingToolDefinitions = [
    {
        ...
        description: "List configured intercompany company pairs with internal Customer (AR) and Supplier (AP) master data links.",
        inputSchema: {
            type: "object",
            properties: {
                company: {
    ...
    },
    {
        ...
        description: "Create intercompany Sales Invoice on the selling company (from_company) — posts to Accounts Receivable (AR).",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Create intercompany Purchase Invoice on the buying company (to_company) — posts to Accounts Payable (AP).",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Create linked Sales Invoice (AR on seller) and Purchase Invoice (AP on buyer) for a company pair.",
        inputSchema: {
            type: "object",
            properties: {
                from_company: { type: "string", description: "Selling company" },
    ...
    },
    {
        ...
        description: "Trace AR/AP posting status for intercompany invoice(s). Resolves linked SI/PI from inter_company_invoice_reference.",
        inputSchema: {
            type: "object",
            properties: {
                sales_invoice: { type: "string", description: "Sales Invoice name (optional)" },
...
async function callIcMethod(client, method, args) {
    return client.callMethod(`${IC_METHOD_PREFIX}.${method}`, args);
}
export async function handleIcBillingToolCall(client, toolName, args) {
    try {
        switch (toolName) {
            ...
            case "ic_create_sales_invoice": {
                if (!args?.from_company || !args?.to_company || !args?.items) {
                    throw new McpError(ErrorCode.InvalidParams, "from_company, to_company, and items are required");
                }
...
export function isIcBillingToolName(name) {
